...
def main():
    print(f"Started username checks at {datetime.now()}\n")
    ...
    found_usernames = []
    attempts = 0
    while len(found_usernames) < 10 and attempts < 100:
        ...
        if 3 <= len(username) <= 20:
            ...
            print(f"[{attempts:02d}] {username} → {status}")
            if is_available:
                found_usernames.append(username)
        time.sleep(0.1)
    if found_usernames:
        print("\n[+] Available usernames found:")
        for i, u in enumerate(found_usernames, 1):
            print(f"{i} - {u} available")
    else:
        print("\nNo usernames found, sorry.")
